...
if ( ! class_exists( 'YITH_WCAF_Payments_Table' ) ) {
	...
	class YITH_WCAF_Payments_Table extends WP_List_Table {
		...
		public function get_bulk_actions() {
			$actions = array(
				'switch-to-completed' => __( 'Switch to completed', 'yith-woocommerce-affiliates' ),
				'switch-to-on-hold' => __( 'Switch to on hold', 'yith-woocommerce-affiliates' ),
		...
		protected function extra_tablenav( $which ) {
			...
			if( $need_reset ){
				echo sprintf( '<a href="%s" class="button button-secondary reset-button">%s</a>', esc_url( add_query_arg( array( 'page' => 'yith_wcaf_panel', 'tab' => 'payments' ), admin_url( 'admin.php' ) ) ) , __( 'Reset', 'yith-woocommerce-affiliates' ) );
			}
		}
