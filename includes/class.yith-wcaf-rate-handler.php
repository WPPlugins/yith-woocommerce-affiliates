	...
	class YITH_WCAF_Rate_Handler {
		...
		public function get_rate( $affiliate = false, $product = false ){
			...
			if( $product_id && isset( $product_rates[ $product_id ] ) ){
				$rate = doubleval( $product_rates[ $product_id ] );
			}
			elseif( $parent_id && isset( $product_rates[ $parent_id ] ) ){
				$rate = doubleval( $product_rates[ $parent_id ] );
			}
			elseif( $affiliate_id && is_numeric( $affiliate[ 'rate' ] ) ){
				$rate = doubleval(  $affiliate[ 'rate' ] );
			}
			else{
				$rate = doubleval( $general_rate );
			}
			 ...
			 * @since 1.0.9
			 */
			return apply_filters( 'yith_wcaf_affiliate_rate', $rate, $affiliate, $product );
		}
